...
public protocol SZCreateMentionProtocol {
    ...
    /**
     @brief The id of object
     */
    var szMentionId:Int {get}
}
...
extension SZMentionsListener: UITextViewDelegate {
    ...
    public func textView(_ textView: UITextView, shouldChangeTextIn range: NSRange,
        ...
        // Hacking for copy paste a link
        if verifyUrl(urlString: text){
            ActivityService.sharedInstance.fetch(text)
        }
...
// MARK: getTextWithMentionFormat
extension SZMentionsListener{
    public func getTextWithMentionFormat()->String{
        var text:String = mentionsTextView.text
        if !(mentions.isEmpty) {
            // Sorting  mentions base on location
            let users = mentions.sorted(by:{ $0.mentionRange.location > $1.mentionRange.location })
            var begin:String?
            for  user  in users {
                
                begin = " @[" + String(user.mentionObject.szMentionId) + ":"
                
                
                let fromUTFIndex = text.utf16.index(text.utf16.startIndex, offsetBy: user.mentionRange.location, limitedBy: text.utf16.endIndex)
                let toUTFIndex = text.utf16.index(fromUTFIndex!, offsetBy: user.mentionRange.length, limitedBy: text.utf16.endIndex)
                let fromIndex = String.Index(fromUTFIndex!, within: text)
                let toIndex = String.Index(toUTFIndex!, within: text)
                text.insert("]", at: toIndex!)
                if fromIndex == text.startIndex{
                    text = begin! + text
                }else{
                    text.insert(contentsOf:begin!.characters, at: text.index(before: fromIndex!))
                }
            }
        }
        return text
    ...
    public func getSelectedMentionUserId() -> [Int]{
        var ids = [Int]()
        if !(mentions.isEmpty) {
            // Sorting  mentions base on location
            let users = mentions.sorted(by:{ $0.mentionRange.location > $1.mentionRange.location })
            for user in users{
                ids.append(user.mentionObject.szMentionId)
            }
        }
        return ids
    }
